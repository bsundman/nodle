...
fn vs_main(vertex: VertexInput, instance: InstanceInput) -> VertexOutput {
    // Expand render quad to accommodate border layer (1px beyond node size on all sides, unscaled)
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // Layer sizing scaled by zoom to match CPU behavior
    let bevel_shrink = 1.0 * uniforms.zoom;      // Bevel shrunk by 1px from border (zoom-scaled)
    let background_shrink = 1.0 * uniforms.zoom; // Background shrunk by 1px from bevel (zoom-scaled)
    let border_expand = 1.0 * uniforms.zoom;     // Border extends 1px beyond node size (zoom-scaled)
    ...
    // Bevel layer: same size as original node (matches CPU behavior)
    let bevel_size = node_size;
    let bevel_pixel_pos = expanded_pixel_pos - vec2<f32>(border_expand, border_expand);
    let bevel_sdf_dist = rounded_rect_sdf(bevel_pixel_pos, bevel_size, corner_radius);
    
    // Background layer: shrunk by additional 1px from bevel (2px total from border)
    ...
    let background_corner_radius = corner_radius - background_shrink; // Shrink radius to match CPU
    let background_sdf_dist = rounded_rect_sdf(background_pixel_pos, background_size, background_corner_radius);
