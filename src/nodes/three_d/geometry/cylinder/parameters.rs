//! Cylinder node parameter interface with primitive/mesh toggle
...
impl CylinderParameters {
    pub fn build_interface(node: &mut Node, ui: &mut Ui) -> Vec<ParameterChange> {
        ...
        if is_primitive_mode {
            ui.label("🔧 Primitive mode: Uses USD procedural cylinder primitive");
        } else {
            ui.label("🔧 Mesh mode: Generates tessellated cylinder mesh");
        }
