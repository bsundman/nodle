//! AND node parameters using Pattern A: build_interface method

use crate::nodes::interface::{NodeData, ParameterChange};
use crate::nodes::Node;
use super::logic::AndLogic;

/// AND node with Pattern A interface
#[derive(Debug, Clone)]
pub struct AndNode {
    pub input_a: bool,
    pub input_b: bool,
...
impl AndNode {
    ...
    pub fn build_interface(node: &mut Node, ui: &mut egui::Ui) -> Vec<ParameterChange> {
        ...
        ui.heading("AND Logic Parameters");
        ui.separator();
        ...
        ui.horizontal(|ui| {
            ...
            if ui.checkbox(&mut invert_result, "Invert output (NAND)").changed() {
                changes.push(ParameterChange {
        ...
        if show_table {
            ...
            let current_b = node.parameters.get("input_b")
                .and_then(|v| if let NodeData::Boolean(b) = v { Some(*b) } else { None })
                .unwrap_or(false);
            let current_invert = node.parameters.get("invert_result")
                .and_then(|v| if let NodeData::Boolean(b) = v { Some(*b) } else { None })
                .unwrap_or(false);
            
            let result = current_a && current_b;
            let final_result = if current_invert { !result } else { result };
    ...
    /// Apply parameters to create AndLogic instance
    pub fn create_logic(&self) -> AndLogic {
        AndLogic {
            a: self.input_a,
            b: self.input_b,
